...
\begin{itemize}
	\item correttezza: valuta se l'algoritmo porta alla soluzione corretta del problema per ogni possibile input
...
\begin{itemize}
	...
	\item un insieme \(\mathcal{S}\) di soluzioni, che rappresentano tutti gli output del problema per ogni istanza
	di \(\mathcal{I}\)
...
\[\begin{array}{ c l }
	\Sigma = \{ 0, \; 1, \; \dots \} & \qquad \text{l'alfabeto dei simboli delle stringhe} \\[6pt]
	\Sigma^0 = \{\varepsilon\} & \qquad \text{l'insieme della sola stringa vuota} \\[6pt]
	\Sigma^i & \qquad \text{l'insieme di tutte le stringhe di lunghezza} \; i \; \text{(potenza cartesiana)} \\[6pt]
	\Sigma^+ = \bigcup_{i = 1}^{\infty} \Sigma^i & \qquad \text{l'insieme di tutte le stringhe esclusa la stringa vuota} \\[6pt]
	\Sigma^* = \bigcup_{i =0}^{\infty} \Sigma^i & \qquad \text{l'insieme di tutte le stringhe inclusa la stringa vuota} \\[6pt]
	\mathcal{I} \subseteq \Sigma^*, \quad \mathcal{S} \subseteq \Sigma^* & \qquad \text{gli insiemi di istanze e soluzioni sono sottoinsiemi di } \Sigma^*
\end{array}\]
La taglia di una istanza è la quantità di informazione necessaria per rappresentarla ed è pari al numero di simboli che
compongono la stringa che la rappresenta, o meglio la sua lunghezza. Per un numero intero \(n\) rappresentato in binario,
la taglia è il numero di cifre che lo compongono \(\approx \log_{2} n\).

\subsubsection*{Osservazioni sulla rappresentazione dei dati nell'insieme delle istanze}
L'insieme delle istanze \(\mathcal{I}\) impone il tipo di rappresentazione in cui i dati in input devono essere codificati.
...
(ad esempio per ridurre la complessità computazionale), ma è sempre necessario specificare la rappresentazione dei dati
in ingresso.
...
Un algoritmo che risolve un problema computazionale definisce una funzione tra l'insieme delle istanze e quello delle soluzioni.
A partire da una istanza, restituirà soltanto una delle possibili soluzioni per tale istanza. È possibile, inoltre, che due
algoritmi diversi risolvono lo stesso problema computazionale, ma producano soluzioni diverse a partire dalla stessa istanza.
...
\[1. \; \mathcal{I}_\text{istanze} \subseteq I_\text{input} \qquad\quad 2. \quad \mathcal{S}_\text{soluzioni} \subseteq O_\text{output} \qquad\quad 3. \quad \forall i \in \mathcal{I} \text{ vale } (i, A(i)) \in \pi\]
...
Il valore ottenuto sarebbe troppo variabile e non permette di stimare in maniera corretta e generale la complessità di un
algoritmo. Per questo motivo si simula l'esecuzione su random access machines con instruction set semplici e tempo di
esecuzione costante per ogni istruzione, usato come unità di misura del tempo di esecuzione. Questa semplificazione
permette sia di stimare la complessità di un algoritmo in maniera più generale, sia di confrontare tra loro algoritmi
diversi prima di implementarli su una macchina reale.
...
Il principio di induzione è definito solo sui i numeri naturali e può essere applicato solo su proprietà che ricevono come
...
di nodi interni) e si mappa ogni cluster ad un numero naturale equivalente proprio al numero di nodi interni. Si applica
quindi il principio di induzione dimostrando che la proprietà vale per ogni cluster.
...
\end{itemize}
per cui si ipotizza che la formula analitica sia un'equazione di terzo grado \[s(n) = a n^3 + b n^2 + c n + d\]
...
\[3a = 1 \Rightarrow a = 1/3 \qquad\quad 3a + 2b = 2 \Rightarrow b = 1/2 \qquad\quad a + b + c = 1 \Rightarrow c = 1/6\]
\end{itemize}
...
per cui i quattro punti scelti non sono sufficienti.

\subsection{Formule matematiche utili}
\vspace{5pt}
\[\begin{array}{c l}
	\displaystyle a \, ^{\log_b c} = c \, ^{\log_b a} & \qquad \text{esponenziale di un logaritmo} \\[10pt]
	\displaystyle \sum_{k=0}^{n-1} r^k = \frac{r^n - 1}{r - 1} & \qquad \text{serie geometrica di ragione \(r\)} \\[20pt]
	\begin{array}{c}
		z = a + ib \;\; \leftrightarrow \;\; z = \rho (\cos \varphi + i \sin \varphi) \;\; \leftrightarrow \;\; z = \rho \cdot e^{i \varphi} \\[5pt]
		\begin{cases}
			\rho = \sqrt{a^2 + b^2} \\
			\varphi = \arctan(b / a)
