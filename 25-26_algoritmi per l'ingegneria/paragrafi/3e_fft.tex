...
\[p(x) \cdot q(x) = \sum_{i=0}^{n+m-2} c_i \, x^i \qquad\quad \text{con} \quad c_k = \!\! \sum_{\substack{i+j=k \\[2pt] 0 \leq i < n \\[2pt] 0 \leq j < m}} \!\! a_i b_j = \sum_{i=0}^{k} a_i \cdot b_{k - i} \qquad \text{per} \; k = 0, 1, \dotsc, n + m\]
...
L'equazione alle ricorrenze e la complessità dell'algoritmo di Horner rimangono lineari nel degree bound del polinomio,
per cui non c'è un netto miglioramento rispetto all'approccio precedente.
\[T(n) \; = \; \begin{cases}
	\; 0 & \text{se } n = 1 \\[3pt]
	\; T(n-1) + 2 & \text{se } n > 1
...
\end{array}\]
È possibile infine eseguire un'ulteriore divisione ottenendo gli otto contributi \(p^{[i][j][k]}(x^8)\), ciascuno di
...
\[p(x) \; = \; \sum_{k=0}^{n-1} \; y_k \, L_k(x) \qquad\qquad L_k(x) \; = \; \frac{\displaystyle \prod_{j \neq k} (x - x_j)}{\displaystyle \prod_{j \neq k} (x_k - x_j)}\]
Si osserva che quando \(x = x_k\), significa che si sta cercando di valutare il polinomio \(L_k(x)\) in un punto \(x_k\),
il polinomio interpolatore \(L_k(x) = 1\) solo per \(k\), mentre per gli altri \(L_i(x) = 0\) e il risultato della
sommatoria è proprio \(y_k\).
...
\subsubsection*{Interpolazione di un polinomio sulle radici dell'unità}
...
La valutazione costa \(\Theta(n \log_2 n)\), si dimostrerà che anche l'interpolazione costa \(\Theta(n \log_2 n)\),
ottenendo così un sistema efficiente per cambiare rappresentazione e ottenere un algoritmo efficiente per il prodotto
di polinomi rappresentati per punti.
...
\begin{itemize}
	\item una radice \(n\)-esima dell'unità è un numero complesso \(z_k = e^{i \varphi}\) con \(\varphi = 2\pi k/n\) e \(k \in [0, \; n\!-\!1]\)
	\[{z_k}^n = [\exp (2 \pi i k / n)]^n = \exp (2 \pi i k / n \cdot n) = \exp (2 \pi i k) = 1\]
	\item una radice \(z_k\) è detta radice principale \(\omega_n\) se elevata per i vari valori di \(k \in [0, \; n\!-\!1]\)
...
\[\begin{array}{c c}
	...
	{\omega_n}^k \cdot {\omega_n}^{-k} = 1 & \qquad \begin{array}{c}\textbf{elemento inverso} \\ \text{l'insieme delle radici \(n\)-esime dell'unità possiede} \\ \text{l'elemento inverso } {\omega_n}^{n-k} \text{ rispetto alla moltiplicazione}\end{array} \\[15pt]
	...
	({\omega_n}^i)^2 = ({\omega_n}^{i + n/2})^2 = {\omega_n}^i & \qquad \begin{array}{c}\textbf{lemma di dimezzamento} \\ \text{le due radici } {\omega_n}^i \text{ e }{\omega_n}^{i + n/2} \text{ sono opposte} \\ \text{ed i loro quadrati sono uguali}\end{array} \\[15pt]
...
\noindent
NOTA: il lemma di somma indica anche la somma degli elementi lungo la righe o la colonna \(i\)-esima della matrice di Fourier,
...
\noindent
La matrice inversa di Fourier è la matrice di Fourier scalata di \(1/n\) in cui le righe da \(1\) a \(n-1\) vengono
invertite e l'insieme dei generatori è l'ultima riga della matrice di partenza.
...
\(-x\) e la si applica per due generiche radici \(n\)-esime dell'unità opposte tra loro (\(\pm \; {\omega_n}^i\)).
\[\left\{ \; \begin{aligned}
...
\end{aligned} \right.\]
\[\text{osservando che} \quad -{\omega_n}^i = {\omega_n}^{i + n/2} \qquad \left({\omega_n}^i\right)^2 = \left({\omega_n}^{i + n/2}\right)^2 = {\omega_{n/2}}^i \qquad \]

\noindent
Si costruisce il vettore dei valori \(\underline{y}\) che assume il polinomio valutato sulle radici \(n\)-esime dell'unità:
...
\end{itemize}
L'equazione alle ricorrenze e la complessità asintotica risultano essere:
\[T(n) \; = \; \begin{cases}
	\; 0 & \text{se } n = 1 \\[3pt]
...
bit-reversal. Tale permutazione è ottenuta invertendo i bit dell'indice \(i\) che identifica la foglia (es. \(i = 6\)
...
Il prodotto di due polinomi \(p(x)\) e \(q(x)\) con degree bound rispettivamente \(n\) e \(m\) è un polinomio
...
Dal prodotto element-wise tra i vettori di punti di \(p(x)\) e \(q(x)\) si otterrà un vettore di punti non sufficientemente
lungo per rappresentare \(r(x)\). È necessario estendere le rappresentazioni tramite vettore di coefficienti al degree bound
\(n + m - 1\) del polinomio risultante, aggiungendo elementi nulli ai vettori di coefficienti di \(p(x)\) e \(q(x)\).
Questa operazione è detta padding dei coefficienti.
\begin{align*}
...
bound del polinomio risultante. Sarà necessario aggiungere ulteriori elementi nulli di padding ai vettori di coefficienti.
\begin{align*}
	\underline{a} = (a_0, a_1, \dots, a_{n-1}, 0, 0, \dots, 0) \; \rightarrow \; \underline{a} = \overbrace{(a_0, a_1, \dots, a_{n-1}, 0, 0, \dots, 0)}^{2^k \, > \, n + m - 1 \; \text{elementi}}) \\
	\underline{b} = (b_0, b_1, \dots, b_{m-1}, 0, 0, \dots, 0) \; \rightarrow \; \underline{b} = \underbrace{(b_0, b_1, \dots, b_{m-1}, 0, 0, \dots, 0)}_{2^k \, > \, n + m - 1 \; \text{elementi}}) \\
\end{align*}
...
\begin{itemize}
	...
	coefficienti di grado superiore nulli
	\item nel mergesort, il padding potrebbe essere fatto con valori facili da discriminare (es. \(-\infty\) o \(+\infty\))
	per cui dopo averli eliminati dal risultato, questo rimane sempre corretto
	\item nella trasformata di Fourier, gli elementi da aggiungere al vettore di coefficienti sono molto complessi da
	determinare, per cui non è possibile applicare un padding in maniera efficiente
\end{itemize}

\noindent
NOTA: il padding di zeri nei vettori di input della DFT per la moltiplicazione di polinomi altera il risultato della DFT
perché banalmente si sta valutando il polinomio su radici dell'unità in numero superiore. Tuttavia, in questo contesto
non è un problema, in quanto entrambe le rappresentazioni (seppur sostanzialmente diverse) sono equivalenti per rappresentare
lo stesso polinomio di partenza.
...
effettuare continui padding per mantenere la dimensione dei vettori di coefficienti sufficientemente grandi per rappresentare
i risultati intermedi e pari ad una potenza di due.

In alternativa a tale processo di padding continuo, si può effettuare un padding iniziale già sufficientemente grande
per garantire che tutti i risultati intermedi e finali siano rappresentabili senza dover effettuare ulteriori padding.

Analizzando la complessità di entrambi i processi, si osserva che il costo complessivo dell'elevamento a potenza è
praticamente lo stesso ed è pari a \(\Theta(2kn \log_2 kn)\), ma che non altera la complessità asintotica.
...
\begin{itemize}
	...
	siccome la somma di tutte le righe della matrice di Fourier è zero, tranne per la prima riga, la cui somma è \(n\) (lemma di somma)
\end{itemize}

\subsubsection*{Composizione di trasformate di Fourier}
La composizione di due trasformate di Fourier \({F_n}^2\) provoca un ribaltamento degli elementi del vettore di input, escluso
il primo elemento, che rimane invariato, a cui si aggiunge uno scalamento di \(n\).
\[{F_n}^2 \cdot \underline{a} = (a_0, a_{n-1}, a_{n-2}, \dots, a_1) \cdot n\]

\noindent
Siccome il ribaltamento applicato due volte riporta gli elementi del vettore di input alla posizione originale, si ha che
la composizione di quattro trasformate di Fourier \({F_n}^4\) provoca soltanto uno scalamento di \(n^2\) senza modificare
la posizione degli elementi del vettore di input.
\[{F_n}^4 \cdot \underline{a} = (a_0, a_1, a_2, \dots, a_{n-1}) \cdot n^2\]
