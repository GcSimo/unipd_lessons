...
\begin{center}
	\begin{tabularx}{\textwidth}{l | l | X}
		\textbf{tipo di segnale} & \textbf{formula matematica} & \textbf{grafico} \\
		\toprule
		segnali sinusoidali & \(\begin{aligned} x(t) &= A \cos(\omega t + \varphi) \\ \omega &= 2 \pi f \quad f = \, ^1\!/\!_T \end{aligned}\) & \includegraphics[width=0.3\textwidth]{sinusoide.png} \\
		\midrule
		\parbox{3.5cm}{segnale esponenziale \\ monotono crescente} & \(x(t) = e^{\sigma t} \quad \sigma > 0\) & \includegraphics[width=0.3\textwidth]{esponenziale crescente.png} \\
		\midrule
		\parbox{3.5cm}{segnale esponenziale \\ monotono decrescente} & \(x(t) = e^{\sigma t} \quad \sigma < 0\) & \includegraphics[width=0.3\textwidth]{esponenziale decrescente.png} \\
		\midrule
		\parbox{3.5cm}{segnale esponenziale \\ costante} & \(x(t) = e^{\sigma t} \quad \sigma = 0\) & \includegraphics[width=0.3\textwidth]{costante.png} \\
		\midrule
		\parbox{3.5cm}{segnale sinusoidale \\ crescente in modo \\ esponenziale} & \(\begin{aligned} x(t) = e^{\sigma t} &A \cos(\omega t + \varphi) \\ \sigma &> 0 \end{aligned}\) & \includegraphics[width=0.3\textwidth]{sinusoide crescente esponenzialmente.png} \\
		\midrule
		\parbox{3.5cm}{segnale sinusoidale \\ decrescente in modo \\ esponenziale} & \(\begin{aligned} x(t) = e^{\sigma t} &A \cos(\omega t + \varphi) \\ \sigma &< 0 \end{aligned}\) & \includegraphics[width=0.3\textwidth]{sinusoide decrescente esponenzialmente.png} \\
		\midrule
		\parbox{3.5cm}{segnale sinusoidale \\ \say{costante in modo \\ esponenziale}} & \(\begin{aligned} x(t) = e^{\sigma t} &A \cos(\omega t + \varphi) \\ \sigma &= 0 \end{aligned}\) & \includegraphics[width=0.25\textwidth]{sinusoide costante.png}
	\end{tabularx}
\end{center}

\subsubsection*{Considerazioni sui segnali sinusoidali}
Un segnale sinusoidale decresente/crescente in modo esponenziale può essere rappresentato attraverso un numero complesso sfruttando
