...
\begin{itemize}
	...
	in the series resistor \(R_S\) even when the load is disconnected, expecially when the load can require a large current \(I_L\)
	\item output voltage \(V_{out}\) changes linearly with variation ofthe load current \(I_L\) due to the not null Zener impedance
...
\noindent
The series resistor \(R_S\) is used to guarantee the minimum current through the Zener diode \(I_Z > I_{Z,min}\):
\[R_S \geq \frac{V_{in} - V_Z}{I_{Z,min} + I_B}\]
	...
	\begin{minipage}{0.8\textwidth}
		\centering \begin{circuitikz}[scale=0.8, transform shape]
			...
			\draw[-{Latex}]
				...
				node[pos=0.35, fill=white, inner sep=3pt] {$V_{BE}$} 
				($(q1b) + (0.2, 0)$);
		\end{circuitikz}
	\end{minipage}
\end{center}
...
a constant voltage \(V_F\) to the partial resistor \(R_2\) of the potentiometer that acts as a voltage divider \(R_1\)
and \(R_2\). The output voltage \(V_{out}\) is given by:
...
across \(R_4\) becomes large enough to turn on the transistor \(Q_3\), which starts to absorbe current from the resistor \(R_3\)
