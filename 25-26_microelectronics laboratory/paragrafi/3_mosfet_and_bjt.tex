...
\begin{center}
	...
	\begin{minipage}{0.45\textwidth}
		\centering \begin{circuitikz}[american]
			...
			% V_GS (Gate-Source Voltage)
			\draw[-{Latex}]
				([xshift=-0.6cm, yshift=0.25cm] pmos.S) to[out=180, in=90] 
				node[midway, fill=white, inner sep=4pt] {$V_{GS}$} 
				([xshift=-0.4cm, yshift=0.25cm] pmos.G);
			
			% V_DS (Drain-Source Voltage)
			\draw[-{Latex}]
				([xshift=0.3cm, yshift=-0.1cm] pmos.S) to[out=-60, in=60] 
				node[midway, fill=white, inner sep=4pt] {$V_{DS}$} 
				([xshift=0.3cm, yshift=0.1cm] pmos.D);
		
			% I_DS Corrente Drain-Source
			\node at ($(pmos.S)+(-0.5, -0.1)$) {\(I_{DS} \downarrow\)}; % Testo I_DS vicino al drain
		\end{circuitikz}\\[5pt]
		PMOS transistor
	\end{minipage}
\end{center}
...
\begin{center}
	\begin{minipage}{0.45\textwidth}
		\centering \includegraphics[width=0.8\textwidth]{immagini/npn_bjt.png}
	\end{minipage}
	\begin{minipage}{0.45\textwidth}
		\centering \includegraphics[width=0.8\textwidth]{immagini/pnp_bjt.png}
	\end{minipage}
\end{center}
...
depencence between the base current \(I_B\) and the collector-emitter voltage \(V_{CE}\) (the same characteristic of a diode).
...
\begin{itemize}
	...
	forward biased; the collector current \(I_C\) depends on the external circuit and previous equations are not valid anymore and
