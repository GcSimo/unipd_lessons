...
\begin{center}
	...
	\end{minipage}
	\begin{minipage}{0.45\textwidth}
		\centering \includegraphics[width=0.7\textwidth]{immagini/inverting_bistable.png}
	\end{minipage}
\end{center}
...
\begin{center}
	...
	\end{minipage}
	\begin{minipage}{0.45\textwidth}
		\centering \includegraphics[width=0.7\textwidth]{immagini/non_inverting_bistable.png}
	\end{minipage}
\end{center}
...
\begin{center}
	...
	\end{minipage}
	\begin{minipage}{0.45\textwidth}
		\centering \includegraphics[width=0.7\textwidth]{immagini/inverting_bistable_offset.png}
	\end{minipage}
\end{center}
...
\begin{center}
	...
	\begin{minipage}{0.35\textwidth}
		\centering \includegraphics[width=0.7\textwidth]{immagini/astable_multivibrator.png}
	\end{minipage}
\end{center}

\subsubsection*{Circuit behavior}
\begin{itemize}
	\item When \(V_{out2}\) is at \(L_+\), there is a constant current \(I_R = L_+ / R\) through the resistor \(R\) that charge
	the capacitor \(C\) at a constant rate of \(L_+ / RC\). (\footnotesize Note: capacitor is charged because the \(I_R\) current
	enters through the positive terminal of the capacitor for the polarity convention used \normalsize )
	\item Due to virtual ground imposed by the left op amp on the positive terminal of the capacitor, the negative terminal
	of the capacitor starts decreasing linearly with a constant rate of \(- L_+ / RC\) as the capacitor voltage \(V_C\)
	increases.
	\item The voltage output \(V_{out1}\) is directly connected to the negative terminal of the capacitor and have the same
	behavior explained in the previous point and demonstrated by the following equations:
	(\footnotesize Note: supposing \(V_{out1}(0) = V_{TH}\) at \(t=0\), corresponding to the time instant when \(V_{out2}\) starts to be at \(L_+\) \normalsize)
\end{itemize}
\vspace{5pt}
\[I_C = C \frac{d \, V_C(t)}{dt} = - C \frac{d \, V_{out1}(t)}{dt} = \frac{L_+}{R} \quad \rightarrow \quad V_{out1}(t) = V_{out1}(0) - \int \frac{L_+}{R} \, dt = V_{TH} - \frac{L_+}{RC} \, t\]
\vspace{0pt}

\begin{itemize}
	\item When the output voltage \(V_{out1}\) reaches the negative threshold voltage \(V_{TL}\) of the bistable circuit, the output
	voltage \(V_{out2}\) switches to \(L_-\) and the current through the resistor \(R\) becomes \(I_R = L_- / R\) (changing sign and
	direction).
	\item The capacitor starts discharging at a constant rate of \(L_- / RC\) and the output voltage \(V_{out1}\) increases linearly
	with a slope of \(-L_- / RC\) until it reaches the positive threshold voltage \(V_{TH}\). At that point, the bistable circuit
	changes the output voltage \(V_{out2}\) back to \(L_+\) and the cycle repeats indefinitely.
\end{itemize}
\vspace{5pt}
\[I_C = C \frac{d \, V_C(t)}{dt} = - C \frac{d \, V_{out1}(t)}{dt} = \frac{L_-}{R} \quad \rightarrow \quad V_{out1}(t) = V_{out1}(0) - \int \frac{L_-}{R} \, dt = V_{TL} - \frac{L_-}{RC} \, t\]
\vspace{0pt}

The period of the output signals is the time it takes for the output voltage \(V_{out1}\) to go from \(V_{TH}\) to \(V_{TL}\)
(during \(T_1\)) and back to \(V_{TH}\) (during \(T_2\)).
\begin{align*}
	\text{at } T_1, \; V_{out1} \text{ reaches } V_{TL}: \quad &V_{out1}(T_1) = V_{TH} - \frac{L_+}{RC} \, T_1 = V_{TL} \quad \rightarrow \quad T_1 = RC \, \frac{(V_{TH} - V_{TL})}{L_+} \\
	\text{at } T_2, \; V_{out1} \text{ reaches } V_{TH}: \quad &V_{out1}(T_2) = V_{TL} - \frac{L_-}{RC} \, T_2 = V_{TH} \quad \rightarrow \quad T_2 = RC \, \frac{(V_{TL} - V_{TH})}{L_-}
\end{align*}
...
The transfer characteristic of the low-pass filter is given by:
...
\[\left| H(j\omega)_\text{filter only} \right| = \left| \frac{1}{\sqrt{1 + (\omega R_4 C_4)^2}} \right| = \frac{1}{\sqrt{2}} \quad \rightarrow \quad \omega = \frac{1}{R_4 C_4} \quad \rightarrow \quad f_c = \frac{1}{2\pi R_4 C_4}\]
