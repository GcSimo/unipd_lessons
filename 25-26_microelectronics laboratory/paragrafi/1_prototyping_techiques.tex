...
\begin{itemize}
	\item \textbf{PCB - Printed Circuit Board}: only the surface with conductive pads for mounting electronic components, along
	with tracks or traces, called metal lines, to connect the components, and ``via holes'' to connect different layers of the
	PCB or to mount components with through-hole technology
...
\begin{itemize}[itemsep=5pt]
	...
	\begin{itemize}[topsep=0pt]
		\item \textbf{FR-4}: organic composite rigid material made on fiberglass cloth with an epoxy resin binder, it is rigid,
	...
	\begin{itemize}[topsep=0pt]
		...
		\item \textbf{rigid-flex}: combination of rigid and flexible sections in the same PCB, usually flex parts are used
...
\begin{itemize}[itemsep=5pt]
	...
	\begin{itemize}[topsep=0pt]
		...
		\item \textbf{SOP - Small Outline Package}: SMT package with pins at 25 mil pitch
		\item \textbf{SSOP - Shrink SOP}: SMT package smaller than SOP
		\item \textbf{TSSOP - Thin SSOP}: SMT package smaller than SSOP
		\item \textbf{QFN / DFN - Dual or Quad Flat No-leads}: SMT package soldered with pads or balls
	\end{itemize}
\end{itemize}
...
Resistor values are standardized in the E-series, where the number \(n\) after ``E'' indicates the number of values per decades
...
\noindent
In the laboratory we use E12 resistors, the most common and widely available. If the required value is not an E12 standard one,
we can combine resistors in series or parallel to obtain the desired value.
...
directly to the dielectric film and everything is rolled up to occupy a small volume. By having a wide surface area, they can
achieve higher capacitance values from \(5 \; pF\) to \(100 \; \mu F\). They have low tolerance, good reliability and high
temperature ratings. They come in different shapes and sizes, normally in rectangular forms.
...
Circuit study consist in solving first order equations and neglecting noise, temperature, parasitic capacitance,
channel length modulation, \dots ~ Computer aided simulations are useful to observe the behavior of circuits
considering also second/third order effects and non-idealities by solving mathematical models of the components provided
