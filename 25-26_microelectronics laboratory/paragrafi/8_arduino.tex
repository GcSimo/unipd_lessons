...
The Atmega328P microcontroller has 32 KB of ISP flash memory for storing code, 2 KB of SRAM for local variables and 1 KB of
EEPROM for non volatile data. It has 23 GPIO (general purpose input/output), 32 general purpose working registers, 6 channels
10-bit ADC, Serial UART, SPI and I\textsuperscript{2}C communication interfaces. It has three timers, internal and
external interrupts and a watchdog timer. It operates at a voltage between 1.8 and 5.5 V.
...
same form factor as the Arduino Nano. It has 14 digital input/output pins, 8 analog input pins and a 64 MHz quartz crystal.
...
\begin{itemize}
	\item Arduino Uno Wifi Rev2: same form factor of the standard Arduino Uno but with an ATmega4809 microcontroller and a
	WiFi module with integrated TCP/IP protocol stack.
	\item Arduino STAR - Otto: equipped with a powerful ARM processor to control power demanding peripherals like LCD, Audio, Camera, etc.
	...
	\item Arduino Giga R1 Wifi and Arduino Portenta H7: boards with an ARM Cortex-M7 microcontroller and a WiFi module,
	for high performance applications like robotics, AI and machine learning
...
The bootloader is a small program stored in a private section of the flash memory that allows the microcontroller to be
programmed through a serial communication protocol. It is responsible also for the execution of the user program and for
the management of the reset and power-on events.

To make programming process easier, Arduino boards come also with a USB-to-serial converter chip that interfaces the bootloader
with the USB communication protocol used by computers. This allows the microcontroller to be programmed with a computer.

Without a bootloader, the only way to program the microcontroller is through the SPI communication pins, which requires
an external programmer board and a more complex programming process. 
...
\noindent
A good practice is to prefer global variables rather than local variables, because first ones are stored in the flash memory and
do not consume SRAM, which is a very limited resource on microcontrollers.
...
\begin{itemize}
	\item \textbf{digital output}: pin configured as a digital output, used to write digital signals (\texttt{LOW} or
	\texttt{HIGH}) through \texttt{digitalWrite(PIN, [HIGH,LOW])} function
	\item \textbf{digital input}: pin configured as a digital input, used to read digital signals (\texttt{HIGH} or
	\texttt{LOW}) through \texttt{digitalRead(PIN)} function
	\item \textbf{analog output}: pin configured as a digital PWM output, used to write PWM signals with custom duty cycle in
	range 0-255 through \texttt{analogWrite(PIN, value)} function, PWM frequency can vary depending on board and pin used
	\item \textbf{analog input}: pin configured as an analog input, used to read analog signals through the ADC and get
...
\begin{itemize}
	...
	is connected to \(V_{CC}\) through a \(\approx 20 \; k\Omega\) pull-up resistor, so the pin reads \texttt{HIGH} when it is
	not connected to ground and \texttt{LOW} when it is connected to ground.
\end{itemize}
...
\[\text{ADC step} = \frac{V_{REF}}{2^{\text{bits}}} \qquad \begin{array}{r l}
	\text{\small for Arduino Uno: } & \displaystyle \text{ADC step} = \frac{5 \; V}{2^{10 \; \text{bits}}} = 4.88 \; mV \\[7pt]
	\text{\small for Arduino Nano 33 BLE Sense: } & \displaystyle \text{ADC step} = \frac{3.3 \; V}{2^{12 \; \text{bits}}} = 0.806 \; mV
\end{array}\]
...
\begin{itemize}
	...
	\begin{itemize}[topsep=-1pt]
		\item \texttt{RISING}: triggers the interrupt when the pin goes from \texttt{LOW} to \texttt{HIGH}
		\item \texttt{FALLING}: triggers the interrupt when the pin goes from \texttt{HIGH} to \texttt{LOW}
		\item \texttt{CHANGE}: triggers the interrupt when the pin changes state (from \texttt{LOW} to \texttt{HIGH} or vice versa)
		\item \texttt{LOW}: triggers the interrupt when the pin is \texttt{LOW} (only for some boards)
		\item \texttt{HIGH}: triggers the interrupt when the pin is \texttt{HIGH} (only for some boards)
	\end{itemize}
\end{itemize}
...
to connect the pin to \(V_{CC}\). In this way, when the button is not pressed, the pin reads \texttt{HIGH} and when the button
is pressed, the pin reads \texttt{LOW}.
...
Usually, when a button is pressed, the transition from \texttt{HIGH} to \texttt{LOW} is not clean and immediate, but there
are some oscillations called bouncing due to mechanical switch contacts oscillations. To solve this problem, a simple software
debouncing technique can be used, which consists in leaving a short time (10 and 50 ms) between two consecutive readings
of the button state.
...
and \(V_{CC}\). The button behavior becomes active \texttt{HIGH}, meaning the state will be \texttt{HIGH} when the button
is pressed and \texttt{LOW} when the button is not pressed. 
...
at the wiper terminal will be between 0 V and \(V_{CC}\) and can be read by an analog pin using the \texttt{analogRead()} function.
...
In some applications, it is necessary to scale the value read from the potentiometer to a different range (for example in range 0-255
to control the brightness of an LED). To do this, it is possible to use the \texttt{map(x, in\_min, in\_max, out\_min, out\_max)}
function to map the value \texttt{x} from the input range \([\texttt{in\_min}, \; \texttt{in\_max}]\) to the output range
...
Asynchronous Serial Protocol is a family of communication protocols that allow to exchange data between the microcontroller
and other devices using a serial communication interface. It has the following characteristics:
\begin{itemize}
	\item asynchronous: the data is transmitted without a clock signal, so the sender and the receiver must agree on the baud
	rate (bits per second) and the format of the data (start and stop bits, parity bit, etc.) before the start of the communications
...
It is a full-duplex protocol that uses two lines for communication: TX (transmit) and RX (receive). The TX line of one device
is connected to the RX line of the other device and vice versa. In idle state, lines are \texttt{HIGH}. The communication has
the following format:
\begin{itemize}
	\item start bit: 1 bit \texttt{LOW}, indicates the start of a data frame
	\item data bits: bits transmitted in little endian order (least significant bit first)
	\item stop bit: 1 bit \texttt{HIGH}, indicate the end of a data frame
\end{itemize}
...
Serial Peripheral Interface (SPI) is a synchronous serial communication protocol that uses a master-slave configuration
to exchange data between a single controller device (master) and one or more controlled peripheral devices (slaves).
...
\begin{itemize}
	...
	\item CS (Chip Select): line used to select a specific peripheral device (active \texttt{LOW})
\end{itemize}
...
\begin{itemize}
	\item the master selects the slave device by pulling its CS line \texttt{LOW}
	\item the master generates the clock signal on the SCLK line
	\item on each falling edge of the clock signal, the master and the slave transmit one bit of data on the CIPO and COPI
	lines that will be read during the next clock period
	\item after the communication is finished, the master stops the clock signal and pulls the CS line \texttt{HIGH}
\end{itemize}
...
\subsubsection*{Library file organization and syntax highlighting}
...
The I\(^2\)C (Inter-Integrated Circuit) is a synchronous serial communication protocol that uses a master-slave configuration
...
\end{itemize}
NOTE: SDA line can change state only when SCL is \texttt{LOW}, except for the start and stop conditions.
...
\begin{itemize}
	\item the master device initiates the communication by sending a start condition (SCL keeps \texttt{HIGH} while SDA goes
	from \texttt{HIGH} to \texttt{LOW}) and starts generating the clock signal on the SCL line
	...
	\item after the communication is finished, the master sends a stop condition (SCL keeps \texttt{HIGH} while SDA goes
	from \texttt{LOW} to \texttt{HIGH}) and stops generating the clock signal on the SCL line
\end{itemize}
...
\begin{itemize}
	...
	\item \texttt{Wire.endTransmission()} sends the bytes enqueued in the buffer and ends the transmission
...
CMOS chip that measure the visible and infrared light intensity with 16-bit resolution. There are two independent ADC channels
...
The sensor has a 7-bit address that can be partially configured using an external pin of the sensor (0x29 when \texttt{LOW},
0x39 when floating, 0x49 when \texttt{HIGH}). I\(^2\)C commands for the TSL2561 sensor are:
...
\begin{center}
	...
	\end{minipage}
\end{center}
It is possible to create custom colors with the \texttt{tft.color565(r, g, b)} library function, where \(r\), \(g\) and \(b\)
...
\begin{itemize}
	...
	\texttt{ts.pressureThreshold} (equal to 0), the touch is considered valid
\end{itemize}
...
\begin{lstlisting}
	...
	// check if a touch event occurred by comparing the pressure value to the threshold
	if (p.pressure > ts.pressureThreshold) {
		// touch event has occurred at coordinates (p.x, p.y) with p.pressure
	}
\end{lstlisting}
...
\begin{enumerate}
	...
	\item a photodiode connected in reverse bias to a transimpedance amplifier (TIA) (opamp 1) with a potentiometer in the feedback loop
	that converts the photodiode current into a voltage signal
...
and deoxygenated hemoglobin have different light absorption coefficients for red and infrared light. The ratio and the oxygen saturation
is given by the following formulas:
\[R = \frac{\displaystyle \left(\frac{AC}{DC}\right)_{RED}}{\displaystyle \left(\frac{AC}{DC}\right)_{IR}} = \frac{(V_{\text{max,RED}} - V_{\text{min,RED}}) \; / \; V_{\text{DC,RED}}}{(V_{\text{max,IR}} - V_{\text{min,IR}}) \; / \; V_{\text{DC,IR}}} \approx \frac{(V_{\text{max,RED}} - V_{\text{min,RED}}) \; / \; V_{\text{min, RED}}}{(V_{\text{max,IR}} - V_{\text{min,IR}}) \; / \; V_{\text{min,IR}}}\]
\[\text{SpO}_2 = 97.94 + 1.15 \cdot R\]
