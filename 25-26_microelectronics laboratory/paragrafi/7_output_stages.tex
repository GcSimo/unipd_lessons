...
needed to preserve the same signal vaweform, but with higher current and power. A good power amplifier should have:
\begin{itemize}
...
\begin{center}
	\begin{minipage}{0.5\textwidth}
		\centering\begin{circuitikz}[american, scale=0.8, transform shape]
			...
			\draw (outSplit) -- ++(0.5,0) to[short, -o, i_=\(I_L\)] ++(0.5,0) node[right] {$V_{out}$};
...
\subsubsection*{With diode to bias transisor}
...
The problem of this configuration is that the current mirror transistos should be matched (should come from the same silicon wafer)
and sometimes this is impossible.
...
lower limit of the output voltage is the minimum (absolute) between \(-V_{CC}+V_{CE2sat}\) (saturation of \(Q_2\)) and
...
In normal operation, when the load is connected and \(I_{gen} = V_{CC} / R_L\) (the maximum load current that absorbe the load
...
is zero. This is the normal resting point of the output stage, so it is one of the most common working points and the transistor
must be able to dissipate the power at this point without overheating.
...
The RMS power drawn from the supply is given by the sum of the power erogated by the negative supply \(I_{gen} \cdot V_{CC}\)
and the power erogated by the positive supply \(I_L \cdot V_{CC}\). For a sinusoidal input signal, the mean load current is
\(I_L = I_{gen}\), so the total power drawn from the supply is:
\[P_S = P_{S, negative} + P_{S, positive} = I_{gen} \cdot V_{CC} + I_L \cdot V_{CC} = 2 I_{gen} \cdot V_{CC}\]
The power conversion efficiency of a class A output stage is therefore:
\[\eta = \frac{P_L}{P_S} = \frac{\frac{1}{2} \cdot \frac{V_{out}^2}{R_L}}{2 I_{gen} \cdot V_{CC}} = \frac{1}{4} \cdot \frac{V_{out}}{I_{gen} \cdot R_L} \frac{V_{out}}{V_{CC}} \leq 0.25\%\]
...
\begin{center}
	\begin{circuitikz}[american, scale=0.8, transform shape]
		...
		\draw ($(output)$) to[short, -o, i_=\(I_L\)] ++(0.7,0) node[right] {$V_{out}$};
...
\begin{center}
	\begin{minipage}{0.5\textwidth}
		\begin{circuitikz}[american, scale=0.8, transform shape]
			...
			\draw (Q1.E) -- node[midway] (output) {} (Q2.E);
			\draw ($(output)$) to[short, -o, i_=\(I_L\)] ++(1,0) node[right] {$V_{out}$};
	...
	\begin{minipage}{0.45\textwidth}
		...
		It is important to keep in mind op amps limitations, expecially low slew rates can limit the compensation.
	\end{minipage}
\end{center}
...
\begin{center}
	\begin{minipage}{0.45\textwidth}
		\centering \begin{circuitikz}[american, scale=0.8, transform shape]
			...
			\draw ($(output)$) -- ++(0.5,0) to[short, -o, i=\(I_L\)] ++(1,0) node[right] {$V_{out}$};
...
\begin{itemize}
	...
	negleting the \({I_Q}^2\) term in the square root, showing that the output stage behaves like a class B output stage when
	the load current is big enough
\end{itemize}

\subsubsection*{Transfer characteristic}
The transfer characteristic of the class AB output stage with ideal bias voltage \(V_{BB}\) is a perferct straight line with a
