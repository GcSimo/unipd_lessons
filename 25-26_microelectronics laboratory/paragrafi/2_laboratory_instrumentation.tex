...
the same impedace. For this reason the real output voltage is doubled to ensure the desired voltage across the load. When
the function generator is connected to a load with a different impedance, the load impeadance must be set accordingly. In
particular, when it is connected to a HiZ load, the instument must be set to HiZ mode.
