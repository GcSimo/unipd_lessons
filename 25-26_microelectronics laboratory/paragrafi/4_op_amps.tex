...
\begin{center}
	...
	\begin{minipage}{0.6\textwidth}
		\begin{itemize}
			...
			\item \(V_{in}\): differentialinput voltage (\(V_{in} = V^+ - V^-\))
...
\begin{itemize}
	...
	\item \textbf{infinite slew rate}: the output voltage can change instantaneously in response to changes in the input voltage
\end{itemize}
...
\begin{center}
	\begin{minipage}{0.45\textwidth}
		...
		\end{circuitikz} \\[5pt]
		\footnotesize Zero output offset voltage \(V_0\) op amp model with input offset voltage \(V_{OS}\) compensation
		hilghlighted in red.
	\end{minipage}
	\hfill
...
Due to finite input resistance, there is a small input bias current \(I_B\) that flows into the input terminals of the op amp.
This is because the input terminals are connected to the base or gate of internal transistors (BJT or MOSFET) that have a non-infinite
resistance. The difference between the two input bias currents is called input offset current \(I_{OS}\).
...
to generate the input bias currents \(I_{B_+}\) and \(I_{B_-}\).
...
add a compensation resistor \(R_{B}\) in series to the non-inverting input with a value equal to the parallel of \(R_{in}\)
...
\begin{center}
	\begin{circuitikz}[american, scale=0.8, transform shape]
		...
		\draw (opamp.+) to[short, -o] ++(0, 0) node[left] {\(V_{in}\)};
		\draw (opamp.-) to[R, l_=$R_{in}$] ++(-2,0) node[ground] {};
		\draw (opamp.out) -- ++(0.7,0) to[short, -o, i_=$I_L$] ++(0.7,0) node[right] {$V_{out}$};
		\draw (opamp.out) ++(-0.4, 0) to[short, i_=$I_{out}$] ++(0.7, 0);

		\draw (opamp.-) to[short, *-] ++(0, 1) coordinate (fb_up)
			to[R, l=$R_{f}$, ] (fb_up -| opamp.out)
			to[short] ++(0.5, 0) coordinate (fb_out)
...
\begin{itemize}
	\item[1.] calculate the minimumequivalent resistance \(R_{eq} = R_L \parallel R_f\) based on \(V_{out, max}\) and \(I_{out, max}\)
...
\end{itemize}
It is recommended to choose resistor values between input impeadance (\(1 \; M\Omega \; - \; 10 \; M\Omega\)) and output impedance
