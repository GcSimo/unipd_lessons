...
\begin{itemize}[topsep=3pt, itemsep=0pt]
	\item[-] L'algoritmo inserisce le chiavi una ad una, incrementando \(j\) e costruisce l'heap con un up-heap bubbling per
...
\begin{algo}{remove}{$k$}{chiave $k$ della entry da rimuovere e riferimento implicito all'ABR $T$}{valore della entry rimossa}
	...
	\If{($T$.isInternal($w$))}
		...
		\If{($T$.isExternal($T$.left($w$)) OR $T$.isExternal($T$.right($w$)))}
			...
			\If{$T$.isExternal($u_L$)} \Comment Caso 1.1: la foglia è il figlio sinistro
...
\end{itemize}
\begin{algo}{ShortestPath}{$G$, $s$, $t$}{grafo $G = (V,E)$ connesso, $s,t \in V$}{lista $L$ con gli archi del cammino minimo da \(t\) a \(s\)}
...
\begin{algo}{MergeSort}{$S$}{come da specifica}{come da specifica}
	...
	\Else
		...
		\For{\(i \gets 0\)}{\(\lceil n/2 \rceil - 1\)} \(S_1[i] \gets S[i]\)
		\EndFor
		\For{\(i \gets \lceil n/2 \rceil\)}{\(n-1\)} \(S_2[i-\lceil n/2 \rceil] \gets S[i]\)
		\EndFor
...
\begin{algo}{Merge}{$S_1$, $S_2$, $S$}{sequenze ordinate \(S_1\) e \(S_2\), sequenza vuota \(S\)}{sequenza ordinata \(S= S_1 \cup S_2\)}
	\State \(i \gets 0, \quad j \gets 0, \quad k \gets 0\)
	\While{(\(i < S_1.\text{size}()\) AND \(j < S_2.\text{size}()\))}
		\If{(\(S_1[i] < S_2[j]\))} \(S[k++] = S_1[i++]\)
		\Else \(S[k++] = S_2[j++]\)
		\EndIf
	\EndWhile
	\While{(\(i < S_1.\text{size}()\))} \(S[k++] = S_1[i++]\)
	\EndWhile
	\While{(\(j < S_2.\text{size}()\))} \(S[k++] = S_2[j++]\)
	\EndWhile
\end{algo}

\subsubsection*{Analisi}
L'algoritmo si basa sul design pattern divide-et-impera. La complessità del metodo Merge è \(\Theta(n)\). Analizzando l'albero
della ricorsione si osserva che ad ogni livello la taglia aggregata di tutte le chiamate ricorsive è \(n\), per cui ogni livello
ha complessità aggregata \(\Theta(n)\). L'albero della ricorsione è un albero binario completo con altezza \(\Theta(\log n)\),
per cui la complessità complessiva è \(\Theta(n \log n)\).

\newpage

\subsection{QuickSort}
\subsubsection*{Algoritmo}
\begin{algo}{QuickSort}{$S$, $a$, $b$}{come da specifica, indici \(0 \leq a,b \leq n-1\)}{come da specifica}
	\If{(\(a \geq b\))} return
	\Else
		\State \(l \gets \) Partition(\(S\), \(a\), \(b\))
		\State QuickSort(\(S\), \(a\), \(l\))
		\State QuickSort(\(S\), \(l+1\), \(b\))
	\EndIf
\end{algo}

\begin{algo}{Partition}{$S$, $a$, $b$}{sequenza \(S\), indici \(0 \leq a,b \leq n-1\)}{indice \(l\) e sequenza \(S\) tale che \(S[i] \leq S[l] \leq S[j]\) per \(a \leq i < l < j \leq b\)}
	\State \(p \gets S[b], \quad l \gets a, \quad r \gets b-1\)
	\While{(\(l \leq r\))}
		\While{(\(l \leq r\) AND \(S[l] \leq p\))} \(l \gets l+1\)
		\EndWhile
		\While{(\(l \leq r\) AND \(S[l] \geq p\))} \(r \gets r-1\)
		\EndWhile
		\If{(\(l \leq r\))} swap(\(S[l]\),\(S[r]\))
		\EndIf
	\EndWhile
	\State swap(\(S[l]\),\(S[b]\))
	\State return \(l\)
...
\begin{algo}{InsertionSort}{$S$}{come da specifica}{come da specifica}
	\For{\(i \gets 1\)}{\(n-1\)}
		\State \(curr \gets S[i]\)
		\State \(j \gets i-1\)
		\While{(\(j \geq 0\) AND \(S[j] \geq curr\))}
