...
\begin{itemize}
	\item \textbf{laziness}: un certo fenomeno può avere molte cause possibili, elencarle tutte richiede troppo tempo e
...
\begin{itemize}
	...
	\item \textbf{assiomi della probabilità}: \(\qquad 0 \leq P(e) \leq 1, \qquad \sum_{e \in W} P(e) = 1\)
...
Ad esempio, per tre variabili aleatorie \(x_1, x_2, x_3\), in base alle indipendenze che sussistono tra le tre variabili,
si può avere che:
...
regole della probabilità e ricorrendo eventualmente a tecniche semplificative come normalizzazione e marginalizzazione.
...
graph'' (DAG)) è una rappresentazione grafica sotto forma di grafo diretto aciclico che modella le relazioni di dipendenza
...
\begin{algo}{Gibbs-Sampling}{\textit{bn}, \textit{num-samples}, $X$, $e$}{\textit{bn}, \textit{num-samples}, $X$ ed $e$ come in precedenza}{$\textbf{P}(X \mid e)$: stima della probabilità a posteriori di \(X\) data l'evidenza \(e\)}
	...
	\For{$i = 1$ to \textit{num-samples}}
		\State \textbf{choose} $Z_i$ tra le variabili di \textit{bn} escluse quelle in $e$, secondo una distribuzione \(\rho(i)\)
...
superiore al primo è utile specificarne anche l'ordine (es. second order Markov process). La rappresentazione grafica
...
L'operazione di filtering si basa sulle equazioni del Kalman filtering, ampiamente diffuso specialmente nei sistemi
di navigazione e posizionamento GPS:
\begin{align*}
...
L'algoritmo del Kalman filtering si divide in due fasi principali:
\begin{itemize}
...
osservazione \(\textbf{O}_t\) quante sono le possibili evidenze \(\textbf{e}_t\) osservabili. Le matrici di osservazione
...
Il Laplacian smoothing consiste nell'inizializzare tutte le distribuzioni di probabilità a probabilità uniformi. Ad esempio
