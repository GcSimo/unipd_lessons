...
\begin{itemize}
	...
	\item ``drownings increase with ice cream consumption'': c'è correlazione tra il numero di annegamenti e il consumo di gelato,
...
\subsubsection*{Problemi di interpretazione della full joint probability e passaggio ai causal model}
...
rappresentano in maniera altrettanto valida la stessa full joint probability.
...
Inoltre, vincolando i valori di \(Z\), l'effetto di \(Z\) su \(Y\) non è più generale, ma dipende dal valore osservato di \(X\).
...
ACE positivo \(\approx 5\%\) cioè significa che assumere il farmaco aumenta la probabilità di guarire del 5\% rispetto a non
assumere il farmaco.
\begin{align*}
...
\begin{itemize}
	...
	al primo passo, escluso il valore imposto a \(X\) 
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{action} o intervention: modificare la SCM intervenendo su \(X\) (eliminandone le dipendenze) e imponendo
...
Gli algoritmi di PC e FCI sono stati progettati per dataset time-invariant, ovvero dataset senza correlazione temporale
...
\begin{itemize}
	\item \textbf{PC algorithm}: si applica il PC algorithm per identificare soltanto lagged dependencies, ovvero le dipendenze
...
da un precedente algoritmo come F-PCMCI o L-PCMCI (più permissivo dell'F-PCMCI). In questo modo, quando si hanno unobservable
...
aggiungendo eventualmente variabili esogene (per rispettare la faithfulness).
