...
Il \textbf{bias} è la misura dell'errore sistematico dovuto alla scelta della classe della funzione ipotesi \(h\): se
la classe è troppo semplice, la funzione ipotesi non riesce a rappresentare correttamente i dati dei training set. Un
modello con un alto bias non si adatta bene ai dati di addestramento e ne sottostima la complessità (ad esempio un modello
lineare per dati non lineari). Viceversa un modello con un basso bias rappresenta bene i dati di addestramento (quando
la classe dell'ipotesi è appropriata ai dati).
...
La \textbf{variance} è la misura di quanto varia la funzione ipotesi \(h\) in funzione delle fluttuazioni nei dati di
addestramento. Un modello con un'alta variance si adatta troppo bene ai dati di addestramento (ad esempio un modello
polinomiale di grado troppo elevato). Viceversa, uno con bassa variance riesce a generalizzare meglio e l'addestramento
con dati diversi produce modelli simili.
...
\begin{itemize}
	...
	\item[*] le cross entropy loss verranno approfondite nella sezione dedicata alle reti neurali (\hyperref[sec:loss_functions]{link})
\end{itemize}
