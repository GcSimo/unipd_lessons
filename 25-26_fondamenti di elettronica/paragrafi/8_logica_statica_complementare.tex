...
\begin{itemize}
	\item per ogni combinazione di ingressi per cui \(F(\dots) = 0\), l'uscita della porta logica deve essere \(0 \V\), ovvero
...
%\subsubsection*{Calcolo delle capacità parassite e modello RC (NAND a 3 ingressi)}
...
Si osserva che per ottimizzare il tempo di ritardo è possibile avere \(Z_p\) e \(Z_n\) minori per i mosfet che sono collegati
direttamente al nodo di uscita (coinvolti nel termine quadratico) e maggiori per i mosfet più lontani dall'uscita.
