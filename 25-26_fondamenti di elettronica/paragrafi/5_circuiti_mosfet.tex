...
\subsubsection*{Circuito n-mosfet e resistenza}
\begin{center}
	\begin{minipage}{0.3\textwidth}
		\centering \begin{circuitikz}[american]
			...
			\draw (G_mosfet) node[nmos, anchor=G](nmos){}; % n-mosfet
	...
	\begin{minipage}{0.57\textwidth}
		...
		\end{itemize}
		L'utilizzo di un n-mosfet al posto di un p-mosfet è perfettamente arbitrario e non cambia nulla nel procedimento di
...
\begin{enumerate}
	...
	velocità) escludendone, se possibile, uno come spiegato al passo precedente, e si risolve la rete come nel modello a canale lungo:
	\begin{itemize}[topsep=0pt]
...
\subsubsection*{Circuito di un n-mosfet connesso a diodo}
\begin{center}
	\begin{minipage}{0.3\textwidth}
		\centering \begin{circuitikz}[american]
			\ctikzset{tripoles/mos style=arrows}
...
\subsubsection*{Circuito di un n-mosfet connesso come generatore di corrente}
...
\noindent
L'utilizzo di un n-mosfet al posto di un p-mosfet è perfettamente arbitrario e non cambia nulla nel procedimento di soluzione
del circuito.
...
\begin{itemize}
	\item una corrente costante \(I_{DSAT}\) di saturazione del mosfet, corrisponde alla corrente erogata dal generatore ideale
...
\begin{center}
	\begin{minipage}{0.37\textwidth}
		...
		\begin{circuitikz}
			...
			\draw (G_mosfet) node[nmos, anchor=G, rotate=-90](nmos){}; % n-mosfet
...
\begin{center}
	\begin{minipage}{0.37\textwidth}
		...
		\begin{circuitikz}
			...
			\draw (G_mosfet) node[pmos, anchor=G, rotate=90, xscale=-1](pmos){}; % p-mosfet con source/drain invertiti
...
\begin{center}
	\begin{minipage}{0.37\textwidth}
		...
		\begin{circuitikz}
			...
			\draw (G_mosfet) node[pmos, anchor=G, rotate=-90](pmos){}; % p-mosfet
...
\begin{center}
	\begin{minipage}{0.37\textwidth}
		...
		\begin{circuitikz}
			...
			\draw (G_mosfet) node[nmos, anchor=G, rotate=90, xscale=-1](nmos){}; % p-mosfet con source/drain invertiti
...
\begin{itemize}
	...
	\item per \(t = 0\): l'nmos si accende poiché \(V_{GS}(t) = V_{DD} > V_{TN}\);
	\item per \(t > 0\): l'nmos è acceso finché \(V_{GS}(t) > V_{TN} \;\; \rightarrow \;\; V_C(t) < V_{DD} -V_{TN}\)
	\item per \(t \rightarrow \infty\): a regime l'nmos si spegne quando \(V_{GS}(t) = V_{TN} \; \rightarrow \; V_C(t) = V_{DD} - V_{TN}\)
...
\subsubsection*{Dati numerici per il calcolo dei tempi caratteristici}
...
\begin{itemize}
	...
	\item il tempo di carica con pmos è circa il doppio rispetto al tempo di scarica con nmos, a parità del fattore di forma
	\(Z_n = Z_p\), differenza dovuta alla minore mobilità delle lacune rispetto agli elettroni.
\end{itemize}
...
\[I_{DSATN} = Z_n I_{DSATN0} \qquad\qquad I_{DSATP} = Z_p I_{DSATP0}\]
...
\[R_{n} = \frac{R_{n0}}{Z_n} \;\; \text{con} \; R_{n0} = \frac{3}{4} \frac{V_{DD}}{I_{DSATN0}} \qquad\qquad R_{p} = \frac{R_{p0}}{Z_p} \;\; \text{con} \; R_{p0} = \frac{3}{4} \frac{V_{DD}}{I_{DSATP0}}\]
...
\[k_n' \approx 3k_p', \quad V_{TN} \approx -V_{TP}, \quad V_{DSATN} \approx -\frac{2}{3} V_{DSATP} \;\; \Rightarrow \;\; R_{p0} \approx 2R_{n0}\]
\[\frac{R_{p0}}{R_{n0}} = \frac{k_n' V_{DSATN} (V_{DD} - V_{TN} - \frac{V_{DSATN}}{2})}{k_p' V_{DSATP} (-V_{DD} - V_{TP} - \frac{V_{DSATP}}{2})} \approx \frac{k_n' V_{DSATN}}{-k_p' V_{DSATP}} \approx 3 \cdot \frac{2}{3} = 2\]
...
\subsubsection*{Serie di mosfet}
La serie di mosfet equivale ad un mosfet equivalente con resistenza equivalente pari alla somma delle resistenze equivalenti
dei singoli mosfet e con fattore di forma pari al reciproco della somma dei reciproci dei fattori di forma dei singoli mosfet:
\begin{center}
	\begin{minipage}{0.4\textwidth}
...
\subsubsection*{Parallelo di mosfet}
La parallelo di mosfet equivale ad un mosfet equivalente con resistenza equivalente pari al reciproco della somma dei reciproci
