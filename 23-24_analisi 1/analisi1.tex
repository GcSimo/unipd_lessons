...
\begin{enumerate}
	...
	\item \(\displaystyle (a + b) ^ n = \sum_{k = 0} ^ {n} \binom{n}{k} \cdot a ^ k \cdot b ^ {n - k}\)
\end{enumerate}

\subsection{Dimostrazione binomio di Newton}
Si dimostra per induzione \dots
...
%		ad ogni elemento \(x \in X\) un unico elemento \(y \in Y\), \(f : X \to Y\) \\
%		\midrule
%		dominio & insieme \(X\) dove è definita la funzione \\
%		\midrule
%		codominio & insieme \(Y\), generalmente è un insieme numerico \(\mathbb{N}, \mathbb{Z}, \mathbb{R}, \dots\) in cui vengono mappati gli 
%		elementi del dominio \\
%		\midrule
%		\multirow{2}{*}{immagine} & sottoinsieme di \(Y\) definito come \(\Img(f) = f(A)\) \\
...
%		\midrule
%		\(f\) a variabili reali & se il dominio \(X \subseteq \mathbb{R}\) \\
%		\midrule
%		\(f\) a valori reali & se il codominio \(Y \subseteq \mathbb{R}\) \\
%		\bottomrule
%	\end{tabularx}
%\end{center}
...
%		\midrule
%		\(f\) inversa & \(f ^ {-1}:f(X) \subseteq Y \to X\) t.c. \(y \mapsto f ^ {-1}\) ovvero l'unico \(x \in X\) t.c. \(f(x) = y\) \\
%		\midrule
%		\(f\) composta & dati \(f:X \to Y\), \(g:V \to Z\) con \(f(X) \cap V \neq \varnothing\), \(\overline{X} \subseteq X\) t.c. \(\overline{X} := \left\{ x \in X \text{ t.c. } f(x) \in V \right\}\),
...
%		\midrule
%		\(f\) periodica & \(f\) è periodica se \(f(x+T) = f(x), \forall x \in \dom f\) per \(T \in \mathbb{R}, T > 0, x + T \in \dom f\) \\
%		\bottomrule
%	\end{tabularx}
%	\begin{tabularx}{\textwidth}{l X}
...
%		\midrule
%		\multirow{2}{*}{\(f\) limita superiormente} & se \(\Img f\) è limitata superiormente, ovvero se \\
%		& \(\exists M \in \mathbb{R}\) t.c. \(f(x) \leq M\), \(\forall x \in \dom f\) \\
%		\midrule
%		\multirow{2}{*}{\(f\) limitata inferiormente} & se \(\Img f\) è limitata inferiormente, ovvero se \\
%		& \(\exists m \in \mathbb{R}\) t.c. \(f(x) \geq m\), \(\forall x \in \dom f\) \\
%		\midrule
%		\multirow{2}{*}{\(f\) limitata} & se \(\Img f\) è limitata superiormente e inferiormente: \\
%		& \(\exists m, M \in \mathbb{R}\) t.c. \(m \leq f(x) \leq M\), \(\forall x \in \dom f\) \\
%		\midrule
%		maggioranti, minoranti, massimi & un maggiorante, minorante, massimo, minimo, estremo inferiore, \\
%		minini, estemi sup. e inf. di \(f\) & estremo superiore di \(f\) è definito come magg\dots di \(\Img f\) \\
%		\bottomrule
%	\end{tabularx}
%\end{center}
...
il limite è \(+\infty\). Quando si indica \say{definitivamente} riferito ad una successione, si intende \say{definitivamente
per \(n \to +\infty\)}.
...
\begin{center}
	\begin{tabular}{c c}
		\begin{tabular}{c c}
			\textbf{funzione} \(f(x)\) & \textbf{derivata} \(f'(x)\) \\
			\toprule
			\(c\) & \(0\) \\
			\midrule
			\(\alpha \; x\) & \(\alpha\) \\
		...
		\begin{tabular}{c c}
			...
			\midrule
			\(\settsinh x\) & \(\displaystyle \frac{1}{\sqrt{x^2-1}}\) \\
			\midrule
			\(\settcosh x\) & \(\displaystyle \frac{1}{\sqrt{x^2+1}}\) \\
			\midrule
			\(\setttanh x\) & \(\displaystyle \frac{1}{1-x^2}\) \\
			\bottomrule
		\end{tabular}
	\end{tabular}
\end{center}
...
\begin{center}
	\begin{tabular}{c c}
		\begin{tabular}{c c}
			\textbf{integrale} \(\displaystyle \int f(x)\) & \textbf{primitiva} \(F(x)\) \\
			\toprule
			\(\displaystyle \int k \; dx\) & \(kx + c\) \\
			\midrule
			\(\displaystyle \int x^\alpha \; dx\) & \(\displaystyle \frac{x^{\alpha + 1}}{\alpha + 1} + c \quad \text{per }\alpha \neq -1\) \\
			\midrule
			\(\displaystyle \int \frac{1}{x} \; dx\) & \(\log \left|x\right| + c\) \\
			\midrule
		...
		\begin{tabular}{c c}
			...
			\midrule
			\(\displaystyle \int \frac{1}{\sqrt{x^2 - 1}} \; dx\) & \(\settsinh x + c\) \\
			\midrule
			\(\displaystyle \int \frac{1}{\sqrt{x^2 + 1}} \; dx\) & \(\settcosh x + c\) \\
			\midrule
			\(\displaystyle \int \frac{1}{1 - x^2} \; dx\) & \(\setttanh x + c\) \\
			\bottomrule
		\end{tabular}
	\end{tabular}
\end{center}
