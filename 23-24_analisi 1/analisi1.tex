...
\(\displaystyle m \cdot (a - b) = L(f, P) \leq \int_{a}^{b} f(x) dx \leq U(f, P) = M \cdot (a-b)\) \\
...
Sia \(f:\left[a, b\right] \to \mathbb{R}\) una funzione R.I., si considera \(\displaystyle F(x) = \int_{a}^{b} f(t) dt\) per \(x \in \left[a, b\right]\),
...
Dato che \(x \in \left(x_0 - \delta, x_0 + \delta\right)\), allora \(\displaystyle F(x) - F(x_0) = \int_{a}^{x} f(t) dt - \int_{a}^{x_0} f(t) dx = \int_{x}^{x_0} f(t) dt\) \\
Dal teorema della media integrale \(\displaystyle \inf_{\left[x_0, x\right]} f \leq \frac{1}{x-x_0} \int_{x_0}^{x} f(t) dt \leq \sup_{\left[x_0, x\right]} f\), \\
dalla definzione di continuità \(\displaystyle f(x_0) - \varepsilon \leq f(c) \leq f(x_0) + \varepsilon\) con \(c \in \left[x, x_0\right] \subset \left(x_0 - \delta, x_0 + \delta\right)\), \\
...
\begin{center}
	\begin{tabular}{c c}
		\begin{tabular}{c c}
			...
			\midrule
			\(\displaystyle \int \sqrt{x} dx\) & \(\displaystyle \frac{1}{2 \sqrt{x}}\) \\
			\midrule
...
allora è anche integrabile in senso improprio su \(\left(a, b\right)\), cioè se \(\displaystyle \int_{a}^{b} \left|f(x)\right| \; dx\) è CV, lo è anche \(\displaystyle \int_{a}^{b} f(x) \; dx\).
...
\subsubsection*{Dimostrazione}
Sia \(F\) una primitiva di \(f\), allora \(G(x) = F(h(x)) - F(g(x))\), derivando si ha \(G'(x) = f(h(x))h'(x) - f(g(x))g'(x)\) \qed
